...
from cradle.semantic_index import SemanticIndexStore, load_semantic_manifest
...
class AxeSemanticSearcher:
    def __init__(
        ...
        *,
        ...
        self._config = config or SemanticSearchConfig(index_dir=Path(index_dir) if index_dir else None)
    ...
    def search(self, query: str, *, limit: int = 5, search_k: int | None = None, task: str | None = None) -> RetrievalResult:
        plan = build_query_plan(query)
        task_name = task or self._index.default_query_task or self._config.default_query_task
        ...
        node_scores = {entity_id: score * 100.0 for entity_id, score in self._index.search_nodes(query_vector, top_k=candidate_count)}
        symbol_scores = {entity_id: score * 100.0 for entity_id, score in self._index.search_symbols(query_vector, top_k=candidate_count)}
    ...
    def _warn_if_index_stale(self, conn: sqlite3.Connection) -> None:
        ...
        manifest = load_semantic_manifest(self._db_path, index_dir=self._config.index_dir)
        if db_updated_at and manifest.get("db_updated_at") and db_updated_at != manifest["db_updated_at"]:
            logger.warning("semantic index appears stale relative to the SQLite graph; rebuild with `cradle semantic-index`")

    def _rerank_symbols(self, conn: sqlite3.Connection, plan, scores: dict[str, float]) -> dict[str, float]:
    ...
    def _rerank_nodes(
        ...
        for symbol_id, score in anchor_symbols:
            ...
            if plan.wants_implementation:
                node_scores[owner_node_id] = node_scores.get(owner_node_id, 0.0) + 16.0 + min(18.0, score * 0.2)
            elif plan.wants_callers:
                node_scores[owner_node_id] = node_scores.get(owner_node_id, 0.0) - 80.0
            else:
                node_scores[owner_node_id] = node_scores.get(owner_node_id, 0.0) + 4.0 + min(8.0, score * 0.15)
            ...
            if plan.wants_callers:
                for call_row in conn.execute(
                    ...
                    node_scores[call_row["caller_node_id"]] = node_scores.get(call_row["caller_node_id"], 0.0) + 48.0
                for reference_row in conn.execute(
                    ...
                    node_scores[reference_row["source_node_id"]] = node_scores.get(reference_row["source_node_id"], 0.0) + bonus
            ...
            if plan.wants_callees:
                for call_row in conn.execute(
                    ...
                    node_scores[call_row["target_node_id"]] = node_scores.get(call_row["target_node_id"], 0.0) + 30.0
...
def _anchor_symbols(conn: sqlite3.Connection, symbol_hits: list[tuple[str, float]], plan, *, limit: int) -> list[tuple[str, float]]:
    if not symbol_hits:
        return []
    rows = _load_rows_by_ids(
        conn,
...
def _node_bonus(row: sqlite3.Row, plan) -> float:
    ...
    if plan.wants_implementation and row["kind"] == "repo":
        bonus -= 6.0
    if plan.wants_implementation and _looks_indirect_node(path, name, summary, description):
        bonus -= 18.0
    return bonus
...
def _looks_indirect_node(path: str, name: str, summary: str, description: str) -> bool:
    ...
    return any(marker in path for marker in path_markers) or any(marker in combined for marker in summary_markers)
